...
export function AuthProvider({ children }) {
  ...
  async function signIn(arg1, arg2) {
    let email, password
    if (typeof arg1 === "object" && arg1 !== null) { ({ email, password } = arg1) } else { email = arg1; password = arg2 }
    if (!email || !password) throw new Error("Email and password are required")
  ...
  // --- Optional: magic link helpers (kept available) ---
  ...
  async function completeEmailLinkSignIn(emailFromInput) {
    const stored = (() => { try { return window.localStorage.getItem("emailForSignIn") } catch { return null } })()
